...
The data was set up using five-fold cross validation, in which four subsets of
...
training set, but the testing data was withheld until after the competition was completed.   
...
components accuracy, and balancing the weights behind the meta-learning components 
decision making process.  To optimize the memory graph the training data was used 
...
Further more using the statistical components word frequency information, the 
...
Selecting the best words to be chosen from the sentence the following three factors based on both the 
...
\BlankLine
	$N \longleftarrow 0$\;
	$D \longleftarrow 0$\;
	\ForAll{$t$ a word-sense of $T$}{ 
		$N \longleftarrow N$ + the number of times $t$ has been in a sentence with $W$\;
...
The method in which graph-based adaptation and query took place was through the use of
...
were seen together was the initial value of the edges. During the second pass 
of the training data, and the test phase the graph was used to discover the best 
word-sense. It did this by begining a search on each node related to the chosen 
best words of a sentence. The algorithm first looked to see if a disambiguation 
of the target word existed in its neighbours in a edge weight descending order, 
if one did not, it traversed the higest weighted edge, and looked at the new node 
...
disambiguation known.  During this phase the outcomes are: 
...
each component has in their answer to choose the final word-sense.      
...
phase is triggered again. The results from this testing phase is compared 
...
with a likely increase of overall accuracy and efficiency. Along with
