\chapter{Background} Theorists have posited that true intelligence would allow
computers to carry on insightful conversations. The conversations would be of
...
canned responses.   \\ NLP requires numerous approaches and techniques to
...
using Hidden Markov Models     \item The connectionist approach; which combines
...
be researched first.   \\ One of the sub-problems of natural language processing
...
hybrid methods \cite{HAWKWSD}.  \\ The last forty years in artificial
...
of possible research avenues. \\ One area that has a rather small volume of work
...
theories are briefly described in the following sections: \section{Baddeley and
Hitch} \begin{figure}[htp]     \begin{center}
\includegraphics[width=0.85\linewidth]{figures/models/BH-Model}
\caption{Baddeley and Hitch Model}     \end{center}     \label{fig:BH_MODEL}
\end{figure} In Baddeley and Hitch's model \cite{Work_Mem, Episodic_Buff}, three
...
\section{Cowan} \begin{figure}[htp]     \begin{center}
\includegraphics[width=0.85\linewidth]{figures/models/Cowan-Model}
\caption{Cowan Model}     \end{center}     \label{fig:COWAN_MODEL} \end{figure}

In Cowan's theory \cite{ACTIVATION, ATTN_MEM}, working memory is part of long-
term memory.  The representation used by working memory is a subset of memory
...
\begin{itemize}     \item Focus is controlled by both voluntary (conscious) and
involuntary (unconscious) methods.      \item Focus is required to perform many
day to day tasks such as: planning, understanding, and cause and effect.
\item The level of control of focus and capacity of focus differ on an
individual basis.  \end{itemize}    This model readily lends itself to solving
problems in both a high-level idea-oriented situation and a low-level detail-
oriented situation, with the ability to change between the two.  Working on and
...
and understanding.  Each application of problem solving then modifies the long-
term memory instance it had used.

\section{Ericsson and Kintsch} \begin{figure}[htp]     \begin{center}
\includegraphics[width=0.85\linewidth]{figures/models/Ericsson-Kintsch}
\caption{Ericsson and Kintsch Model}     \end{center}     \label{fig:EK_MODEL}
\end{figure} In this theory, Ericsson and Kintsch \cite{LTM} hypothesized that
...
\section{Additional Models} In addition to the three well known theories
...
\cite{SMARTUNCONSCIOUS}. The effects of the unconscious has in its majority
...
completely related to explicit memory \cite{ImplicitMem}. \\ While little work
...
expands on what attention and focus is and the importance of it to our memory.
...
and Kintsch also helps explain how the mind and memories perform some tasks so
...
when unconscious direction are considered.
