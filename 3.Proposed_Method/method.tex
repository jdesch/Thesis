...
lower algorithm run-times.  The process involves removing stopwords, then normalizing and stemming 
...
remaining words would be transformed to lowercase, punctuation removed, and passed to the stemmer. 
The output would be: ``enjoy bass tast delici". This sentence is more compact then the original 
...
statistical information to select the words in the sentence which have the greatest chance of 
successfully completing the disambiguation. After the best words are selected, they are loaded 
into a memory data structure. This data structure acts as working memory for the course of 
solving the disambiguation, the changes to this structure to solve the problem are recorded with 
the goal of adding useful information to the long-term memory structure, to better future queries. 
This working memory connects the current problem to existing problems, 
allowing for information from previously captured disambiguations to be activated and used in
problem-solving algorithms.  To determine the word-sense, two functions begin disambiguating the 
sentence, identified by the two arrows pointing to ``Complete?''.  The memory component is used 
by the search function, which simulates a cognitive thought process by searching through 
working memory and activating the required long-term memory components until a result is found. 
The other function is found in the statistics engine, and is a fast statistical algorithm used 
as a best effort guess on the words chosen from the ``best word choice'' algorithm. Once these 
functions have completed, they return a word-sense and an associated confidence score.
These two sets of results are then passed to the ``return answer'' section which uses a 
meta-component to determine which word-sense is to be the final answer and what of the 
collected working-memory, combined with the feedback from the answer given should be used to 
update both the statistical component and the long-term memory data structure. After the 
process is complete a check is done in the statistical engine to determine how well the 
system is optimized, seen in the ``update Meta-AI?'', if the check determines changes should 
...
and the interface which communicates with long-term memory and the statistical component. 
...
\begin{enumerate}     
	...
	and how often the word has lead to a successful disambiguation.     
	\item a best effort guess method that given nothing but the target word, 
	it returns the most likely disambiguation.
\end{enumerate}
...
\begin{enumerate}
	\item a method to get or learn the various words.  This method activates each word from the problem set,
	bringing its neighbours into working memory. If a word does not exist in long-term memory, a graph of its 
	neighbours found in the problem set are loaded.  The word would be added through the memory object after 
	the completion of the problem.  
	\item a voting mechanism. This method votes on which is the correct word-sense by traversing the memory
	graph. Each word in the problem set returns the first possible disambiguation of the target found. The disambiguation
	found the most is voted as correct. 
...
\begin{enumerate}     
	\item dimensionality - Currently the long-term memory object is setup to solve just the 
	disambiguation problem, no other data outside this problem is collected 
	...
	found in one of the additional dimension of data collected, allowing for better 
	accuracy.
...
allow the current model to better learn how to learn. It adjusts how it problem 
solves based on trial and error. The tracked values are values that can affect 
